...
public:
    ...
    #if PLATFORM_WINDOWS
        // ::UUID layout: Data1(4) | Data2(2) | Data3(2) | Data4[8]
...
// IMPORTANT:
//  * Put this AFTER the full definition of Magic::UUID
//  * Inside namespace std
//  * operator() must be const and noexcept
namespace std {
template<>
struct hash<Magic::UUID>
{
    size_t operator()(const Magic::UUID& id) const noexcept
    {
        ...
        // load as two 64-bit words (unaligned-safe on x86/x64; if you want portable, memcpy)
        std::uint64_t lo, hi;
        std::memcpy(&lo, bytes.data() + 0,  8);
        std::memcpy(&hi, bytes.data() + 8,  8);
        std::uint64_t h = mix64(lo) ^ mix64(hi + 0x9e3779b97f4a7c15ull);
