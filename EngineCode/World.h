                ...
                case EntityType::StaticMesh:
                {
                    ...
                    for (const SubMeshData& subMeshData : pStaticMeshData->m_subMeshes)
                    {
                        ...
                        if (hasDiffuseTexture) // TODO:
                        {
                            ...
                            // pSubMesh->diffuseImage = GRenderer->CreateGPUOnlyImage(imci);
                            // constexpr size_t bytesPerChannel = 1;
                            // const size_t dataSize = extent.width * extent.height * subMeshData.materialData.diffuseData.numChannels * bytesPerChannel;
                            // AllocatedBuffer stagingBuffer = GRenderer->UploadBuffer(dataSize, pStaticMeshData->textureData.data() + subMeshData.materialData.diffuseData.baseTextureDataOffset, VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
