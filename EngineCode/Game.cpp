...
void Game::PostShutdown(GPUContext& gpuctx)
{
    VkDevice device = gpuctx.GetDevice();
    vkDeviceWaitIdle(device);
    m_pipeline.Destroy();
    vkDestroySemaphore(device, m_timelineSemaphore, nullptr);
    vkDestroyImageView(device, m_colorImageView, nullptr);
    vmaDestroyImage(gpuctx.GetVmaAllocator(), m_colorImage, m_colorImageAllocation);
}

Game::Game() { }
Game::~Game() { }

void Game::PreInitialize(GPUContext& gpuctx, const Swapchain& swapchain)
{
    VkDevice device = gpuctx.GetDevice();
    ...
    VkShaderModule vs_m = gpuctx.CreateShaderModule(vspv);
    VkShaderModule ps_m = gpuctx.CreateShaderModule(pspv);

    VkFormat outRTFormats[] = { swapchain.GetFormat() };
    ...
    pipelineBuilder.SetExtent(gpuctx.outputWidth, gpuctx.outputHeight);
    ...
    {
        VkExtent3D extent = {.width = static_cast<uint32_t>(gpuctx.outputWidth), .height = static_cast<uint32_t>(gpuctx.outputHeight), .depth = 1 };
        ...
        VK_CHECK(vmaCreateImage(gpuctx.GetVmaAllocator(), &colorImageInfo, &vmaAllocInfo, &m_colorImage, &m_colorImageAllocation, nullptr));
...
void Game::DoGPUWork(GPUContext& gpuctx, Swapchain& swapchain)
{
    GPUContext::PerFrameInFlightData frameData = gpuctx.GetFrameInFlightData(m_frameNumber);
    VkDevice device = gpuctx.GetDevice();
    ...
    const Swapchain::SwapchainImageData swapchainImageData = swapchain.GetNextSwapchainImageData(frameData.m_imageReadySemaphore);
    ...
    }
    {
        ...
        auto ri = TEMP_rendering_info_fullscreen(1, &rai, nullptr, gpuctx.outputWidth, gpuctx.outputHeight);
        vkCmdBeginRendering(cmdBuf, &ri);
        const VkViewport DEFAULT_VIEWPORT_FULLSCREEN = { 0.0f, 0.0f, static_cast<float>(gpuctx.outputWidth), static_cast<float>(gpuctx.outputHeight), 0.0f, 1.0f };
        const VkRect2D DEFAULT_SCISSOR_FULLSCREEN = { {0, 0}, {static_cast<uint32_t>(gpuctx.outputWidth), static_cast<uint32_t>(gpuctx.outputHeight)}};
    ...
    }
    {
        const VkImageCopy imageCopy= {
            ...
            .extent = {
                .width = static_cast<uint32_t>(gpuctx.outputWidth),
                .height = static_cast<uint32_t>(gpuctx.outputHeight),
                .depth = 1
            }
        };
    ...
    {
        m_timelineValue++;
        gpuctx.SignalFrameInFlight(m_frameNumber, m_timelineValue);
        ...
        vkQueueSubmit(gpuctx.m_graphicsQueue, 1, &submitInfo, nullptr);
    }




    VkSwapchainKHR s = swapchain.GetSwapchainHandle();
    ...
    vkQueuePresentKHR(gpuctx.m_graphicsQueue, &presentInfo);
}
